...
| [`marshal-files/`](./marshal-files) | The MARSHAL source tree for *this* product repo. In a consumer repo this same tree is installed as `.marshal/`. Holds entrypoint, config, knowledge, references, design, plus `marshal-files/{skills,skills-fallback,agents,rules}/` (the canonical built-in MARSHAL skills, subagents, and rules — all named `marshal-*`) and `marshal-files/extensions/{skills,agents,rules}/` (repo-specific extensions, all named `mx_*` at creation). |
...
| [`.cyncia/`](./.cyncia) | Wrapper directory exposing the sync engine. Contains two relative symlinks — `scripts/` and `skills/` — pointing into [`.cyncia-source/`](./.cyncia-source). The `agent-conf-sync` skill should invoke `.cyncia/scripts/sync-all.sh` (or `sync-all.ps1`) from the repo root. |
| [`.cyncia-source/`](./.cyncia-source) | Vendored submodule — the actual sync engine sources. Treat as read-only. The wrapper above is what tooling should reference. |
...
- **Built-in MARSHAL assets** (lifecycle skills, MARSHAL subagents, rules) use the `marshal-` prefix. They are authored under `marshal-files/{skills,skills-fallback,agents,rules}/`. `marshal-promote-assets` reprefixes them with `mx_` when copying into `.agent-config/`.
- **Repo-specific MARSHAL extensions** generated by `marshal-learner` (or hand-authored) live under `marshal-files/extensions/{skills,agents,rules}/` and use the `mx_` prefix **at creation time**. `marshal-promote-assets` copies them into `.agent-config/` as-is (no double-prefix). They survive MARSHAL upgrades because they live outside the built-in folders.
