...
2. **Create the `.marshal/` skeleton:**
   ...
   - `extensions/{skills,agents,rules}/` (each with a `.gitkeep`),
     ...
     `marshal-learner` (or hand-authored), all `mx_`-prefixed at
...
6. **Promote MARSHAL durable assets into `.agent-config/`** by running
   ...
   - the built-ins (`skills/`, `skills-fallback/`, `agents/`, `rules/`)
     — copied into `.agent-config/{skills,agents,rules}/` with the
     `mx_` prefix applied at copy time, and
   - the extensions (`extensions/{skills,agents,rules}/`) — already
     `mx_`-prefixed at creation, copied as-is (empty at init time).
